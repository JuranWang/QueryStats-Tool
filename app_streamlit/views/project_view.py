...
st.caption(f"{project['source_lang']} → {project['target_lang']} · 建于 {project['created_at']}")

tab_quant, tab_qual, tab_desk = st.tabs(["定量问卷调研", "定性访谈", "案头研究"])

with tab_quant:
    st.caption("这个项目历史上分析过的每份问卷，按最近更新排在最前面。")

    documents = db.list_documents(conn, project_id, research_type="quant_survey")

    if not documents:
        st.info("这个项目下还没有问卷分析。")
    else:
        for doc in documents:
            col_title, col_meta, col_open, col_rename, col_delete = st.columns([3, 2, 1, 1, 1])
            col_title.markdown(f"**{doc['title']}**")
            col_meta.caption(f"更新于 {doc['updated_at']}")
            if col_open.button("打开", key=f"open_doc_{doc['id']}"):
                st.session_state["current_document_id"] = doc["id"]
                st.session_state["analysis_mode"] = "load_existing"
                st.switch_page("app.py")

            with col_rename.popover("重命名"):
                new_title = st.text_input("新标题", value=doc["title"], key=f"rename_doc_input_{doc['id']}")
                if st.button("保存", key=f"rename_doc_save_{doc['id']}"):
                    if new_title.strip():
            ...
            with col_delete.popover("删除"):
                st.warning(f"确定删除「{doc['title']}」这份问卷分析？不可恢复。")
                if st.button("确认删除", key=f"delete_doc_confirm_{doc['id']}", type="primary"):
                    db.delete_document(conn, doc["id"])
                    st.rerun()

    st.divider()
    if st.button("+ 新建问卷分析", type="primary"):
...
with tab_qual:
    st.info(
        "定性访谈还没接进这个工具——已经有独立的 interview-crosstab skill（自包含 HTML，不需要这个"
        "工具的后端），架构完全不同，这里先占个分类位置，不强行合并。"
    )

with tab_desk:
    st.info("案头研究还没做，先占个分类位置。")
