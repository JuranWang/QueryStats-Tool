...
界面默认是**英文**，右上角（Deploy 旁边）有个 **EN／中文** 开关可以切换，会记住你的选择。

当前版本号见 [`VERSION`](VERSION)；每个版本改了什么见 [`CHANGELOG.md`](CHANGELOG.md)
（新的排最上面）。
...
- 一个 LLM 供应商的 API key（可选，不配也能用——只是不能用 AI 翻译/分类/洞察这几个
  功能，纯统计图表和 Word/PDF/Markdown 导出不需要 API key）

## 支持的题型

单选、多选、排序、开放、数值题，每种都配了合适的图表（排序题是每个选项一张饼图，
加一张"选项 × 名次"的汇总表格）。同一个项目里上传新文件，会自动套用上一次保存时的
题型和题目文本，不用每次重新配置数据映射表。导出支持 Word / PDF / Markdown 一键生成。
...
- 这是公开仓库，任何有 GitHub 账号的人都能直接提交 Issue，不需要作者邀请。
